...
#include <linux/list.h>        // For list_head and list iteration macros (and hlist macros)
#include <linux/slab.h>        // For kmalloc/kfree (though not strictly needed here)
...
static int (*original_writepage)(struct page *page, struct writeback_control *wbc);

// Our custom writepage function
static int custom_writepage(struct page *page, struct writeback_control *wbc)
{
    printk(KERN_INFO "WRITING: %p\n", page);
    // You can add your custom logic here
    return original_writepage(page, wbc);
}
		...
		if (path.dentry && path.dentry->d_sb) 
		{
			   ...
			   s_readonly_remount != sb->s_readonly_remount)
			{
				...
				else 
				{
					...
					struct inode *inode;
					struct list_head *p;
					printk(KERN_INFO "scan started");
					list_for_each(p, &sb->s_inodes) 
					{
						inode = list_entry(p, struct inode, i_sb_list);
						
						if(atomic_cmpxchg(&inode->i_writecount, 0, 0) == 0)
						{
							;
						}
						else
						{
							printk(KERN_INFO "Inode number: %lu at:%d dirt:%d wc:%d\n", inode->i_ino, inode->i_ctime.tv_sec, inode->dirtied_when, inode->i_writecount);
       					}
       				}
				    printk(KERN_INFO "scan ended");
				    */
...
static void iterate_superblocks_for_fs_type(struct file_system_type *fs_type, int hook)
{
    struct super_block *sb;
    unsigned int count = 0;

    if (!fs_type) {
        pr_err("iterate_superblocks_for_fs_type: fs_type is NULL.\n");
        return;
    }

    pr_info("Iterating superblocks for filesystem type: %s\n", fs_type->name);

     struct address_space_operations *aops;
     
      hlist_for_each_entry(sb, &fs_type->fs_supers, s_instances) 
      { // Corrected from list_for_each_entry to hlist_for_each_entry
     
     	aops = sb->s_bdev->bd_inode->i_mapping->a_ops;
     	if(hook)
     	{
     		pr_info("hooks api %p\n", sb);
     		original_writepage = aops->writepage;
            // Overwrite with our custom function
            aops->writepage = custom_writepage;
     	}
     	else
     	{
     		pr_info("un-hooks api %p\n", sb);
     		aops->writepage = original_writepage;
     	}
     
        // Here, 'sb' points to the current super_block in the list.
        pr_info("  Found superblock: %p, device: %s, state: 0x%lx\n",
                sb, sb->s_id, sb->s_flags); // s_id is the device name, s_flags are mount flags
        count++;
    }
    /*
     * Release the spinlock after the iteration is complete.
     */
    //spin_unlock(&sb_lock);

    pr_info("Finished iterating superblocks for %s. Total found: %u\n", fs_type->name, count);
}
...
static int __init list_files_init(void)
{
    ...
    struct file_system_type *ext4_fs_type = get_fs_type("ext4"); // Example: Get the ext4 filesystem type

    if (ext4_fs_type) {
        iterate_superblocks_for_fs_type(ext4_fs_type, 1);
        // put_filesystem(ext4_fs_type); // Decrement refcount obtained by get_fs_type
    } else {
        pr_info("ext4 filesystem type not found.\n");
    }
...
static void __exit list_files_exit(void)
{
    ...
    if (ext4_fs_type) {
        iterate_superblocks_for_fs_type(ext4_fs_type, 0);
       // put_filesystem(ext4_fs_type); // Decrement refcount obtained by get_fs_type
    } else {
        pr_info("ext4 filesystem type not found.\n");
    }
...
// sb->s_bdev->bd_inode->i_mapping->a_ops;
...
An inode represents each file on a filesystem (although
#endif
